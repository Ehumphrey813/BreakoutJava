...
public class Model 
{
    ...
    {       
        ...
        for (int k=0; k < NUM_BRICKS; k++) {
            GameObj brick = new GameObj(BRICK_WIDTH*k, WALL_3, BRICK_WIDTH, BRICK_HEIGHT, Color.GREEN);
            bricks.add(brick);    
        }
        for (int l=0; l < NUM_BRICKS; l++) {
            GameObj brick = new GameObj(BRICK_WIDTH*l, WALL_4, BRICK_WIDTH, BRICK_HEIGHT, Color.YELLOW);
            bricks.add(brick);    
        }
    ...
    // update lives
    public synchronized void decreaseLives(int p)    
    {
        lives += p;        
    }
